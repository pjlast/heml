...
let rec convert_els_to_ocaml_code (acc : code_fragment list)
  ...
  | h :: tl ->
      convert_els_to_ocaml_code
        ( match h with
        | Ocaml_code c ->
            convert_els_to_ocaml_code (Code_fragment c.code :: acc) []
        | Text t ->
            convert_els_to_ocaml_code
